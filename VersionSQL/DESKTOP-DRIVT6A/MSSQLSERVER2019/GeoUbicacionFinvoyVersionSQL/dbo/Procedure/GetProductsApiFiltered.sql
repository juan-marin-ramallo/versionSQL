/****** Object:  Procedure [dbo].[GetProductsApiFiltered]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[GetProductsApiFiltered]
...
BEGIN
    ...
    SELECT  P.[Id], P.[Name], P.[Identifier], P.[BarCode],P.[Indispensable], 
			PC.[Id] AS ProductCategoryId, PCL.[Id], PC.[Name] AS ProductCategoryName, P.[ImageArray]
	...
	WHERE ((@ProductsId IS NULL) OR dbo.CheckValueInList(P.[Id], @ProductsId)=1)  AND
		  ((@ProductCategoryId IS NULL) OR EXISTS 
		  (
		  ...
		  (@IncludeDeleted = 1 OR (@IncludeDeleted = 0 AND P.[Deleted] = 0))
		  AND (@IdProductBrand IS NULL OR dbo.CheckValueInList(IIF(B.[Id] IS NULL, 0, B.[Id]), @IdProductBrand) = 1) AND
		  ((@ProductBarCodes IS NULL) OR (dbo.CheckVarcharInList(P.[BarCode], @ProductBarCodes) = 1)) AND
		  ((@ProductIdentifiers IS NULL) OR (dbo.CheckVarcharInList(P.[Identifier], @ProductIdentifiers) = 1))
	...
	GROUP BY	P.[Id], P.[Name], P.[Identifier], P.[BarCode],P.[Indispensable], 
				PC.[Id], PCL.[Id], PC.[Name], P.[ImageArray],
