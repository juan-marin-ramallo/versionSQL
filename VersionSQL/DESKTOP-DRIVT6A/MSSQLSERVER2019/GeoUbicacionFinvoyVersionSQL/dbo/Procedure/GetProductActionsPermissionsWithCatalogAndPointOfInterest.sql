/****** Object:  Procedure [dbo].[GetProductActionsPermissionsWithCatalogAndPointOfInterest]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================    
CREATE PROCEDURE [dbo].[GetProductActionsPermissionsWithCatalogAndPointOfInterest]     
  @ProductActionPoiPermissionIds [sys].[VARCHAR](MAX)      
AS     
BEGIN      
    SET NOCOUNT ON;     
  
 SELECT DISTINCT CPOIP.IdPersonOfInterestPermission  
 ...
 --INNER JOIN dbo.CatalogPointOfInterest AS CPOI WITH (NOLOCK) ON CPOI.IdCatalog = C.Id  
 WHERE C.Deleted = 0   
 AND  dbo.CheckValueInList (CPOIP.IdPersonOfInterestPermission, @ProductActionPoiPermissionIds) = 1   
 ORDER BY CPOIP.IdPersonOfInterestPermission  
END
