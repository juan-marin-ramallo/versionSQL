/****** Object:  Procedure [dbo].[GetPointsOfInterestVisitedExcel]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
CREATE PROCEDURE [dbo].[GetPointsOfInterestVisitedExcel]
(
	...
	*/
	...
	BEGIN
		...
		SELECT	POIV.[Id] as IdPointOfInterestVisited, S.[Id] AS IdPersonOfInterest, S.[Name] AS PersonOfInterestName, 
				...
				POI2.[Id] AS HierarchyLevel2Id, POI2.[Name] AS HierarchyLevel2Name,
				...
				POIV.[ConfirmedVisit] AS ConfirmedVisit
		...
		GROUP BY  POIV.[Id], S.[Id], S.[Name], S.[LastName], [IdLocationIn], POIV.[LocationInDate], [IdLocationOut], POIV.[LocationOutDate], 
					...
					POI1.[Id], POI1.[Name], POI2.[Id], POI2.[Name],CAT.[Name], ISNULL(CAO.[Text], CAV.[Value]), CAV.IdCustomAttributeOption, CAT.[Id], POIV.[ConfirmedVisit],
					P.[ContactName],P.[ContactPhoneNumber]
		
		ORDER BY POIV.[LocationInDate] ASC,POIV.[LocationOutDate] ASC, IdPointOfInterestVisited ASC
	...
	BEGIN
		...
		(
			...
			WHERE	(POIV.DeletedByNotVisited = 0 AND ((POIV.[LocationOutDate] IS NULL AND POIV.[LocationInDate] >= @DateFrom AND POIV.[LocationInDate] <= @DateTo) 
						...
						OR (POIV.[LocationOutDate] BETWEEN @DateFrom AND @DateTo)))
					AND [dbo].IsVisitedLocationInPointHourWindow(POIV.[IdPointOfInterest], POIV.[LocationInDate], POIV.[LocationOutDate]) = 1
			GROUP BY	POIV.[Id], POIV.[IdLocationIn], POIV.[LocationInDate], POIV.[IdLocationOut], 
						POIV.[LocationOutDate], POIV.[IdPointOfInterest], POIV.[IdPersonOfInterest], POIV.[ElapsedTime],
						POID.[ConfirmedVisit]
		)
		,vPoints AS
